			...
			switch (input) {
				...
				case "init":
					if (!initialized) {
						...
						if (cacheType == 1)
							fa = new faCache(size, cacheLines, blockSize);
						else if (cacheType == 2)
							dm = new dmCache(size, cacheLines, blockSize);
						else
							nw = new nwCache(size, cacheLines, blockSize);
						initialized = true;
						System.out.println("\n" + cacheName + " has been initialized!");
						if (cacheType == 2)
							System.out.println("All values in the DM Cache have been initalized to 0!");
					} else
				...
				case "type":
					if (cacheType == 0)
						System.out.println("Invalid request, choose a cache first!");
					else {
						if (cacheType == 1)
							System.out.println("FA Cache");
						else if (cacheType == 2)
							System.out.println("DM Cache");
						else
							System.out.println("N-way set Cache");
					}
					break;
				case "write":
					if (cacheType == 0) {
					...
					System.out.println("Formatted 32-bit address: <b" + addressWrite + ">");
					System.out.println();
					switch (cacheType) {
						case 1:
				...
				case "read":
					...
					System.out.println("Formatted 32-bit address: <b" + addressRead + ">");
					System.out.println();
					switch (cacheType) {
						case 1:
	...
	void read(String address, int time) {
		...
		for (tagBlock x : table) {
			if (tag.equals(x.tag)) {
				...
				System.out.println("Address : <b" + address.substring(0, 32 - bbits) + " "
						+ address.substring(32 - bbits, 32) + "> || Value : " + x.blk.data[offset]);
				x.blk.time = time;
			}
		}
		if (!found) {
...
class dmCache extends helper {
	...
	void read(String address, int time) {
		...
		if (tag.equals(table.get(idx).tag)) {
			...
			System.out.println("Address : <b" + tag.substring(0, tag.length() - clbits) + " "
					+ tag.substring(tag.length() - clbits, tag.length()) + " " + address.substring(32 - bbits, 32)
					+ "> || Value : " + table.get(idx).blk.data[offset]);
			table.get(idx).blk.time = time;
		} else {
			System.out.println("Read miss!"); // if read miss, load into cache with default value
			this.write(address, 0, time, true);
...
class nwCache extends helper {
	...
	void write(String address, int val, int time, boolean isRead) { // NW CACHE
		String tag = address.substring(0, 32 - bbits);
		int offset = toDeci(address.substring(32 - bbits, 32));
		table.get(toDeci(tag.substring(tag.length() - log2((cl / n)), tag.length()))).write(address, val, time, isRead,
				bbits, b, offset, log2(cl / n));
	}

	void read(String address, int time) {
		String tag = address.substring(0, 32 - bbits);
		int offset = toDeci(address.substring(32 - bbits, 32));
		table.get(toDeci(tag.substring(tag.length() - log2((cl / n)), tag.length()))).read(tag, offset, time, address,
				bbits, b, log2(cl / n));
	}
...
class sets { // each set inside n-way cache
	...
	void write(String address, int val, int time, boolean isRead, int bbits, int b, int offset, int log2cln) { // Writing
																												// in NW
																												// Set
		String tag = address.substring(0, 32 - bbits);
		boolean found = false;
		...
		if (table.size() < size) {
			...
			System.out.println("Cache Full!");
			System.out.println(
					"Data block with tag <b" + table.get(idx).tag.substring(0, table.get(idx).tag.length() - log2cln)
							+ "> has been evicted! [LRU in SET]");
			table.remove(idx);
			table.add(new tagBlock(tag, new block(time, b)));
			table.get(table.size() - 1).blk.data[offset] = val;
		}
	...
	void read(String tag, int offset, int time, String address, int bbits, int b, int log2cln) {
		...
		for (tagBlock x : table) {
			if (tag.equals(x.tag)) {
				...
				System.out.println("Address : <b" + tag.substring(0, tag.length() - log2cln) + " "
						+ tag.substring(tag.length() - log2cln, tag.length()) + " "
						+ address.substring(address.length() - bbits, address.length()) + "> || Value : "
						+ x.blk.data[offset]);
				x.blk.time = time;
			}
		}
